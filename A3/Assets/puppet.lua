...
grey = gr.material({0.8, 0.8, 0.8}, {0.5, 0.5, 0.5}, 10)
skin = gr.material({1.0, 0.8, 0.6}, {0.0, 0.0, 0.0}, 10)
ugly = gr.material({0.8, 0.8, 1.0}, {0.0, 0.0, 0.0}, 10)
brown = gr.material({0.6, 0.3, 0.0}, {0.0, 0.0, 0.0}, 10)

--foot_test_joint = gr.mesh( 'cube', 'test' )
--foot_test_joint:translate( 0, -5.7 - .5, 0 )
--rootnode:add_child( foot_test_joint )
--foot_test_joint:set_material( red )

test_joint = gr.mesh( 'sphere', 'bbb' )
rootnode:add_child( test_joint )
test_joint:scale( 2.1, 5.8, 1.15 ) -- better bounding box 
test_joint:set_material( blue )

-- JETPACKS --
l_jetpack = gr.mesh( 'sphere', 'l_jetpack' )
rootnode:add_child( l_jetpack )
l_jetpack:scale( 1.0, 2.0, 0.5 )
l_jetpack:translate( 0.0, 1.2, -1.125 ) 
l_jetpack:set_material( grey )
...
r_foot:set_material( brown )
r_foot:scale( 0.8, 0.4, 1.5 )
r_foot:translate( 0.5, -5.5, 0.3 ) 

rootnode:translate(0.0, 5.7 + 0.5 + 1, 0.0)
